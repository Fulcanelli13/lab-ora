...
# Source chapter numbers. Psalms follow Crampon/Hebrew numbering: canonical Douay 44→45, 131→132.
REQUIRED={
 "Luke":[1,2,3,4,9,15,22,23,24], "John":[1,2,3,6,18,19,20],
 "Matthew":[3,5,6,10,16,17,26,27,28], "Mark":[1,16],
 "1 Corinthians":[11], "Isaias":[53], "Acts":[1,2], "Psalm":[45,132],
 "Canticles":[2,6], "Apocalypse":[11,12], "Judith":[13,15]
}
CANONICAL_CHAPTER_ALIASES={"Psalm 44":"Psalm 45","Psalm 131":"Psalm 132"}
ALIASES={
...
def main():
 ...
 for canonical, chapters in REQUIRED.items():
  source=resolved[canonical]; source_ch={n(c.get("chapter")):c for c in source.get("chapters",[])}; out[canonical]={}
  for ch in chapters:
   if ch not in source_ch: raise SystemExit(f"missing chapter {canonical} {ch}")
   verses=[]
   for v in source_ch[ch].get("verses",[]):
    verse_no=n(v.get("verse")); text=str(v.get("text","")).strip()
    if not text: source_blanks.append(f"{canonical} {ch}:{verse_no}"); continue
 ...
 meta={
  ...
  "source_book_names":{k:v.get("name") for k,v in resolved.items()},"canonical_chapter_aliases":CANONICAL_CHAPTER_ALIASES,
  "blank_source_entries":source_blanks,
  "known_source_shape_notes":["Judith 13:23–31 are present inside the database record keyed as Judith 13:20; cue extraction must use verified verbatim substrings."]
 }
 payload={"metadata":meta,"chapters":out}; OUT.parent.mkdir(parents=True,exist_ok=True)
